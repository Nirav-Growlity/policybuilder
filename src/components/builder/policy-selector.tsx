...
import { ArrowRight, BadgeIndianRupee, Leaf, ShieldCheck, Users } from "lucide-react";
import { POLICY_PROFILES } from "@/lib/constants";
import type { PolicyType } from "@/lib/types";

const ICONS = { Leaf, Users, BadgeIndianRupee };

export function PolicySelector({ onSelect }: { onSelect: (type: PolicyType) => void }) {
  return (
      ...
      <section className="relative min-h-screen flex items-center px-7 lg:px-12 py-28">
        ...
        <div className="relative max-w-6xl w-full mx-auto">
          <div className="max-w-2xl animate-fade-up">
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--color-muted)]">New document</p>
          ...
          <div className="mt-14 border-y border-[var(--color-line)] divide-y lg:divide-y-0 lg:divide-x lg:grid lg:grid-cols-3 animate-fade-up stagger-2">
            {Object.values(POLICY_PROFILES).map((profile, index) => {
              const Icon = ICONS[profile.icon];
              return (
                <button key={profile.id} type="button" onClick={() => onSelect(profile.id)} className="group text-left px-0 py-7 lg:px-7 lg:py-1 min-h-[210px] flex flex-col justify-center transition-colors hover:bg-white/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-4" style={{ "--tw-ring-color": profile.accent } as CSSProperties}>
                  ...
                  <h2 className="font-display text-[25px] leading-none mt-7 font-semibold tracking-tight">{profile.short}</h2>
                  <p className="text-[12.5px] leading-[1.55] text-[var(--color-ink-2)] mt-3">{profile.id === "environmental" ? "Environmental stewardship, targets and compliance." : profile.id === "living-wage" ? "Fair compensation, wage benchmarks and worker wellbeing." : "Dignity, safe work and responsible labour practices."}</p>
