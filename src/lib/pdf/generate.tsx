...
function SectionContent({ section, context }: { section: DocumentRenderSection; context: RenderContext }) {
  ...
  switch (content.type) {
    ...
    case "quantitative": {
      ...
      return <View><Text style={{ ...styles.paragraph, color: model.theme.colors.muted, fontSize: 8.5 }}>Targets are tracked against a defined period or reported annually as ongoing commitments.</Text>{model.dataTreatment === "clean-bullets" && model.theme.layout.dataLayout === "target-bands" && section.density !== "dense" ? targets.map((target, index) => <TargetBand key={index} target={target} index={index + 1} context={context} />) : model.dataTreatment === "clean-bullets" && model.theme.layout.dataLayout === "quiet-rules" && section.density !== "dense" ? targets.map((target, index) => <JournalTarget key={index} target={target} context={context} />) : model.dataTreatment === "clean-bullets" ? targets.map((target, index) => <EditorialRow key={index} index={index + 1} title={target.area} body={`${target.target}\n${target.reportingFrequency === "Annually" ? "Reported annually" : `Baseline ${target.baseline || "-"} · Due ${target.deadline || "-"}`}`} context={context} />) : <PdfTable headers={["#", "Focus Area", "Target", "Baseline", "Deadline", "Reporting"]} rows={targets.map((target, index) => [String(index + 1), target.area, target.target, target.reportingFrequency === "Annually" ? "-" : target.baseline, target.reportingFrequency === "Annually" ? "-" : target.deadline, target.reportingFrequency || "Target period"])} widths={[5, 18, 33, 13, 13, 18]} context={context} />}</View>;
    }
    case "responsibilities":
      if (model.dataTreatment === "formal-tables") return <PdfTable headers={["Role / Department", "Responsibility"]} rows={content.entries.map((entry) => [entry.role, entry.duty])} widths={[30, 70]} context={context} />;
