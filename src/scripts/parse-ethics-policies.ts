...
function prompt() {
  return `You are an expert policy-document parser. Extract the Ethics Policy text into valid JSON only. Preserve source paragraphs verbatim; never summarize or invent. Do not include table-of-contents, acknowledgements, signatures, or revision history. Keep all meaningful commitments. Extract quantitative targets only when measurable or time-bound; preserve complete wording and include baseline/deadline only where stated.
...
async function parse(file: string) {
  ...
  const defaultProfile = getPolicyProfile("ethics");
  ...
  if (fs.existsSync(seed)) {
    ...
    existing = {
      ...
      summary: `Parsed Ethics Policy template from ${path.basename(file)}.`,
      tagline: "Ethics Policy",
      policy: {
        policyType: "ethics",
        company: baseObj?.policy?.company || { name: company, industry: "", site: "", sites: [], docNum: "ETH-001", revNum: "01", effectiveDate: "", reviewDate: "", approver: "" },
  ...
  existing.summary = `Parsed Ethics Policy template from ${path.basename(file)}.`;
...
async function main() {
  const candidates = roots
    ...
    .filter(file => /\.(docx|doc)$/i.test(file) && !/^~\$|^~wrl/i.test(path.basename(file)) && relevant.test(path.basename(file)) && !exclude.test(path.basename(file)));

  console.log(`Found ${candidates.length} candidate Ethics documents.`);
  ...
  console.log(`Completed Ethics policy seeds in this batch.`);
}
